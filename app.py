...
def display_audio_report(
    all_opportunities, script_content, audio_file_bytes, pdf_bytes
):
...
with col_password_mode:
    ...
    if password_mode == "Fichier PDF par l'IA":
        uploaded_password_file = st.file_uploader(
            "📁 Charger le Fichier PDF contenant le code", type="pdf"
        )
    else:
        st.markdown("🤫 **Saisir le Mot de Passe (4 caractères)**")
        manual_password = st.text_input(
            label="", type="password", placeholder="Entrez le code ici..."
        )
    ...
    if start_btn:
        ...
        try:
            ...
            if password_mode == "Saisie directe (4 caractères)":
                ...
                with st.status(
                    ...
                    st.write(
                        f"✅ Mot de passe extrait par Gemini : ['{password_content}']"
                    )
                    status.update(
            ...
            with st.status(
                ...
                if pdf_reader.is_encrypted:
                    ...
                    st.warning(
                        "Le Journal PDF n'est pas chiffré. L'analyse continue..."
                    )
                    uploaded_pdf.seek(0)
            ...
            with st.expander("🔍 Aperçu des pages analysées", expanded=False):
                st.write(
                    "Les pages sont affichées ici au fur et à mesure de l'analyse."
                )
        ...
        if all_opportunities:
            ...
            if audio_file_bytes and pdf_bytes and receiver_email:
                ...
                with st.spinner(
                    f"4/4 - Envoi de l'Email automatique à **{receiver_email}** en cours..."
                ):
            ...
            tab_visuel, tab_audio, tab_data = st.tabs(
                ["📱 Vue Cartes", "🎧 Rapport Audio", "📊 Tableau Détails"]
            )
            ...
            with tab_visuel:
                ...
                if sort_option == "Date la plus proche":
                    final_df = filtered_df.sort_values(
                        by="date_dt", ascending=True, na_position="last"
                    )
                elif sort_option == "Date la plus lointaine":
                    final_df = filtered_df.sort_values(
                        by="date_dt", ascending=False, na_position="last"
                    )
                else:
                ...
                if final_df.empty:
                    ...
                    st.info(
                        f"Affiche **{len(final_df)}** opportunités sur **{len(df_opps)}** au total."
                    )
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "secteur": st.column_config.TextColumn(
                            "Secteur", help="Domaine d'activité"
                        ),
            ...
            st.warning(
                "Aucune opportunité pertinente trouvée aujourd'hui. Vous pouvez consulter l'édition d'hier !"
            )

elif uploaded_pdf is not None or (
    uploaded_password_file is not None
    or (manual_password and len(manual_password) == 4)
):
    st.warning(
        "Veuillez charger **le Journal PDF** ET fournir **le Mot de Passe** (par fichier ou par saisie) pour commencer l'analyse."
    )
